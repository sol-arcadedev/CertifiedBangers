...
// AniList" fallback. Mirrors the real /titles/[id] page's layout so
// browsing feels the same either way, but sourced live from AniList
// (no reviews/ratings/seals exist here, since there's no local Title
// row until an admin actually imports it).
export default async function AniListTitlePreviewPage(
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-6 py-8">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-4">
            {isAdmin ? (
              <form
                action={async () => {
                  "use server";
                  await importAniListTitleFromBrowse(anilistId);
                }}
              >
                <button
                  type="submit"
      ...
      <div className="mt-6 rounded-md border border-black/[.08] p-4 text-sm text-zinc-600 dark:border-white/[.145] dark:text-zinc-400">
        Not yet in our catalog — no reviews, ratings, or seals here yet.
        {isAdmin
          ? " Adding it creates the real page here, with the review form ready to go."
          : " Check back later, or let an admin know it's worth adding."}
      </div>
